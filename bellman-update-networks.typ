...
$ Loss^"ADPI"_theta := -E_(History_t^n sim Buffer)[ sum_(t=1)^(T-1) log Prob_theta (Act^n_t |
History^n_t) + log Prob_theta (Rew^n_t, Ter^n_t, Obs^n_(t+1) | History^n_t,
Act^n_t)] $

Here, $History^n_t$ is a trajectory drawn from the $n^"th"$ task/agent pair:
...
Note that the dataset $Buffer$ from which the trajectory $History^n_t$ is drawn
contains multiple tasks and agents, but as the superscript indicates, each
trajectory corresponds to only one. Here we use the term "agent" to denote the
set of policies emitted by a single learning algorithm. Henceforth in this
chapter, we omit the $n$ superscript for the sake of brevity and assume that
there is one task and agent per trajectory. In the preceding chapter, we
...
We now describe the data used to train the Bellman Update Network, though we
defer the details of training to a later section. We assume that we are given a
dataset of trajectories containing observations $Obs$, actions $Act$, rewards $Rew$,
terminations $Ter$, and policy logits $Policy(dot.c|Obs)$. We assume that this
dataset contains a multitude of tasks and agents, with one task/agent per
trajectory. We also assume that we have estimates of value at different numbers
of Bellman updates $k$, although we defer the explanation of how to acquire
these to @sec:train-bellman-network.

In the preceding chapter, an agent corresponded all policies generated by a
learning algorithm, starting with random policies and ending with near optimal
policies. In some sections of this chapter, we will assume one policy per agent,
...
interpolation factor of $.5$, which we chose based on hyperparameter search.
...
#figure(
  ...
  placement: bottom,
  caption: [Regret of improved policy in the $5 times 5$ grid-world, for different values of $delta$ and
...
We refer to this metric as the "regret of the improved policy." Note that this
bears some resemblance to the procedure described in @sec:downstream and
  ...
  figure(
    grid(
      ...
      [#figure(
          align(
            center,
            image("figures/bellman-update-networks/walls-rmse.png", height: 80pt),
          ),
      ...
      [#figure(
          align(
            center,
            image("figures/bellman-update-networks/walls-regret.png", height: 80pt),
          ),
          caption: [Regret of improved policy on $5 times 5$ grid-world with walls.],
        )<fig:walls-regret>],
    ),
...
the interpolation factor from 0.5 to 0.001, again using hyperparameter search.
While lower values of $delta$ clearly outperform higher values in
@fig:bootstrap-rmse, we do observe some overfitting toward the end of training
for $delta=1$.
...
settings of $delta$. The error bands denote the standard error computed across
20 seeds. We also compare the results of training on different quantities of
data by stopping the training of the source algorithm at different points in
time. For reference, the middle graph in @fig:miniworld, trained on 24,576
timesteps of data, terminates training just before the source algorithm reaches
optimal performance.
...
the area under the curve for CQL and $delta=delta_max$ is smaller, perhaps
reflecting limitations in the ability to generalize to the mixture policy
observed during downstream evaluation. We also observe a slight advantage for $delta=1$ over $delta=2$ in
